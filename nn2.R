...
colSums(is.na(eddat))
table(na.omit(dm6[,c(9,12,13,16,17,19,20,22,23,24,14,25,15,18,21,29,30)])$group)#,11,15,18,21,29

dm <- na.omit(dm6[,c(9,12,13,16,17,19,20,22,23,24,14,25,15,18,21,29,30)])
dm$group <- as.factor(dm$group)

dm <- cbind(dm["group"], scale(dm[,c(2:13)]) )
...
for (i in seq(100))
{
  ...
  dm.navg <- avNNet(dm[c(2:13)], dm$group, subset = inTrain, size = 2, rang = 0.5, decay = 5e-4, maxit = 200, repeats = 50)
